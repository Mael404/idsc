    ...
    public function store(Request $request)
{
    ...
    // Default values for discount and fees
    $discountValue = 0;
    $tuitionFeeDiscount = $tuitionFee;
    $miscFee = 0;

    // Calculate discount
    $balanceDue = null; // We'll set this explicitly for "Top Notcher" cases
    if ($request->scholarship !== 'none' && $request->scholarship !== null) {
        $scholarship = Scholarship::find($request->scholarship);

        if ($scholarship) {
            // Check if scholarship is "Top Notcher" (case-insensitive)
            if (stripos($scholarship->name, 'Top Notcher') !== false) {
                $discountValue = $tuitionFee; // Full discount
                $tuitionFeeDiscount = 0; // No tuition fee left to pay
                $miscFee = 0; // No miscellaneous fee
                $balanceDue = 0; // Ensure balance due is 0
    ...
    // Calculate misc_fee if not "Top Notcher"
    ...
    // Initial payment and old accounts default to 0
    $initialPayment = 0;
    $oldAccounts = 0;

    // Calculate total assessment and balance due
    if ($balanceDue === null) {
    ...
    // Insert StudentCourses (as before)
